...
export default function CategoryProductBrowser({ categoryName, products, theme }: ProductBrowserProps) {
  ...
  return (
    <section
      className="py-[112px] max-[760px]:py-20 px-[clamp(22px,4.5vw,76px)] max-[760px]:px-5 bg-[#fbfaf5] gs-reveal scroll-mt-24"
    ...
    >
      <div className="grid grid-cols-[minmax(0,1fr)_minmax(280px,0.55fr)] max-[820px]:grid-cols-1 gap-12 items-end">
        ...
        <label className="grid gap-2.5">
          <span className="text-[clamp(12px,0.85vw,15px)] font-bold tracking-[0.025em] text-muted">Search {categoryName}</span>
          <input
            className={`h-14 px-4 border border-line rounded-md outline-none bg-white text-ink text-[clamp(15px,1.05vw,18px)] transition-[border,box-shadow] focus:ring-4 ${styles.focus}`}
            type="search"
            value={query}
      ...
      <div className="mt-12 pb-4 border-b-2 border-ink flex items-center justify-between gap-5 text-[clamp(13px,0.92vw,16px)] text-muted" aria-live="polite">
        <p className="m-0">
          {filtered.length
        ...
        {query && (
          <button className="pb-1 border-b border-current cursor-pointer bg-transparent text-ink text-[clamp(13px,0.92vw,16px)] font-black" type="button" onClick={() => { setQuery(""); setPage(1); }}>
            Clear search
          </button>
        )}
      </div>
      ...
      {pageCount > 1 && (
        <nav className="mt-10 py-5 border-y border-line grid grid-cols-[1fr_auto_1fr] max-[580px]:grid-cols-2 items-center gap-5" aria-label={`${categoryName} catalogue pages`}>
          <button
            className={`w-fit min-h-11 px-4 border rounded cursor-pointer text-[clamp(12px,0.85vw,15px)] font-black disabled:cursor-not-allowed disabled:opacity-40 disabled:bg-transparent disabled:text-muted disabled:border-line ${styles.button}`}
            type="button"
          ...
          <p className="m-0 text-muted text-[clamp(12px,0.85vw,15px)] max-[580px]:col-span-2 max-[580px]:row-start-1 max-[580px]:text-center">Page <strong className="text-ink">{currentPage}</strong> of {pageCount}</p>
          <button
            className={`w-fit min-h-11 px-4 border rounded cursor-pointer justify-self-end text-[clamp(12px,0.85vw,15px)] font-black disabled:cursor-not-allowed disabled:opacity-40 disabled:bg-transparent disabled:text-muted disabled:border-line ${styles.button}`}
            type="button"
            onClick={() => changePage(currentPage + 1)}
